...
class Burp_api(object):
    ...
    def start_burp(self, burp_api_jar_file,protocol="http",host_ip="127.0.0.1",port="8090",headless_mode="true",java_home="",):
        self.protocol=protocol
        self.host_ip=host_ip
        self.port=port
        self.headers={'Content-Type': 'application/json','Accept': '*/*'}
        if not java_home:
            subprocess.Popen("java -jar "+burp_api_jar_file+" --port="+port+" --headless.mode="+str(headless_mode),shell=True, stdin=subprocess.PIPE, stderr=subprocess.STDOUT, stdout=subprocess.PIPE, close_fds=True)

        if java_home:
            subprocess.Popen(java_home+"/bin/java -jar "+burp_api_jar_file+" --port="+port+" --headless.mode="+str(headless_mode), shell=True, stdin=subprocess.PIPE, stderr=subprocess.STDOUT, stdout=subprocess.PIPE, close_fds=True)
    ...
    #Generate Report in HTML & XML Format after the active scan is completed
